...
def reads_to_axis_obj(a_read: TRRead,
    ...
    shapes += [make_line(-a_read.length * 0.01,
                         unit.start,
                         -a_read.length * 0.01,
                         unit.end,
                         width=5,
    ...
    traces += [make_scatter([(unit.start + unit.end) / 2
                             for unit in a_read.units],
                            [-b_read.length * 0.01
                             for unit in a_read.units],
                            text=a_hov_texts,
                            col=a_cols),
               make_scatter([-a_read.length * 0.01
                             for unit in b_read.units],
...
def reads_to_matrix_obj(a_read: TRRead,
    ...
    return ([make_scatter(x, y, text=texts, col=cols,
                          marker_size=3,
